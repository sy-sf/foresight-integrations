...
def _state_dir() -> str:
    ...
    if not plugin_data:
        ...
        plugin_data = os.path.join(os.path.expanduser("~"), ".claude", "plugins", "data", "foresight-memory")
...
def mark_session_retained(session_id: str) -> None:
    ...
    def _update(data):
        ...
        data[session_id] = now
        # Prune stale entries
        stale = [k for k, v in data.items() if isinstance(v, (int, float)) and v < cutoff]
        for k in stale:
            del data[k]
        return data, None

    _locked_read_modify_write("retained_sessions.json", "retained_sessions.lock", _update)
