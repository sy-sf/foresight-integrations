#!/usr/bin/env bash
# Launch the Foresight MCP server inside the plugin's persistent venv.
...
if [ -z "${PY}" ]; then
  ...
  resolve_py
  if [ -z "${PY}" ]; then
    echo "[Foresight MCP] venv create failed: no Python interpreter in the venv" >&2
    exit 1
  fi
fi
...
# Claude Code launches MCP servers from the active project. Run from the
# plugin-owned data directory so a project-local .env cannot affect FastMCP's
# startup settings, while hooks still receive the project cwd in their input.
mkdir -p "${CLAUDE_PLUGIN_DATA}"
